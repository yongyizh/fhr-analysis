#!/usr/bin/env bash
# Rebuild the stereo_v7 snippet set funet-v24 trained on, from the raw Banner recordings.
#
...
poetry -P "$ROOT" run fhr-snippets "$BASEDIR/training_clips_v7.yaml" \
  --out-dir="$BASEDIR/training/stereo_v7/" --no-plots
