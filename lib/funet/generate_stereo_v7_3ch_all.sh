#!/usr/bin/env bash
# Build stereo_v7_3ch_all (patients 6, 7 AND 8 all in fetal-train/) from the raw Banner recordings.
#
...
poetry -P "$ROOT" run fhr-snippets "$BASEDIR/training_clips_v7_3ch_all.yaml" \
  --out-dir="$BASEDIR/training/stereo_v7_3ch_all/" --no-plots --jobs "${SNIPPET_JOBS:-8}"
