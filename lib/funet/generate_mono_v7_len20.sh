#!/usr/bin/env bash
# Rebuild the mono_v7_len20 snippet set funet-v24 trained on, from the raw Banner recordings.
#
...
poetry -P "$ROOT" run fhr-snippets "$BASEDIR/training_clips_mono_v7_len20.yaml" \
  --out-dir="$BASEDIR/training/mono_v7_len20/" --no-plots --jobs "${SNIPPET_JOBS:-8}"
