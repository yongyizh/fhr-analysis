#!/usr/bin/env bash
# Rebuild the stereo_v7_3ch snippet set funet-v24 trained on, from the raw Banner recordings.
#
...
poetry -P "$ROOT" run fhr-snippets "$BASEDIR/training_clips_v7_3ch.yaml" \
  --out-dir="$BASEDIR/training/stereo_v7_3ch/" --no-plots --jobs "${SNIPPET_JOBS:-8}"
