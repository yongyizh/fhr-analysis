...
def save_interactive(out_html, title, t_a, y_a, name_a, t_b, y_b, name_b,
    ...
    it under ~max_points. `beats_a` / `beats_b` are optional beat-time arrays drawn as
    dashed vertical lines in their own series' colour, each on that series' y-axis.
    """
    ...
    fig.add_trace(go.Scattergl(x=tb, y=yb, name=name_b,
                  ...
                  secondary_y=True)
    for beats, yd, name, color, second in ((beats_a, ya, name_a, "green", False),
                                           (beats_b, yb, name_b, "royalblue", True)):
        if beats is None or not len(beats):
            continue
        bx, by = _beat_segments(beats, yd)
        if bx.size:
            # hoverinfo="skip": see save_interactive_multi -- the segment endpoints are
            # the trace's min/max, not data
            fig.add_trace(go.Scattergl(x=bx, y=by, mode="lines", name=f"{name} beats",
